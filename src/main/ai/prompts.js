export function getSystemPromptOpenAI(target) {
	let sectionName;
	switch (target) {
		case 'balance':
...
export function promptEngineerCleanerOpenAI(contentToClean, target, units) {
	let sectionName;
	switch (target) {
		case 'balance':
			sectionName = 'Balance Sheet';
...
export function promptEngineerSubmitterOpenAI(text, period) {
